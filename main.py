...
def create_tables():
    ...
    CREATE TABLE IF NOT EXISTS sales(
        ...
        client_ip TEXT DEFAULT 'localhost'
    )
    """)
...
def read_sales():
    try:
        ...
        for col in ["opening","closing","litres","price","total","paytm","sbi","hppay","advance","creditor","balance","time_in","time_out","hours","nozzle","client_ip"]:
            if col not in df.columns: df[col]=0
        return df
    except:
        create_tables()
        return pd.DataFrame(columns=["id","date","staff","nozzle","fuel","opening","closing","litres","price","total",
                                     "paytm","sbi","hppay","advance","creditor","balance","time_in","time_out","hours","client_ip"])
    ...
    if st.button("Save Entry"):
        try:
            ...
            """,(str(sale_date),staff,nozzle_int,fuel,opening,closing,litres,price,total,
                 paytm,sbi,hppay,advance,creditor,balance,t1.strftime("%H:%M"),t2.strftime("%H:%M"),hours,ip_address))
            conn.commit()
            st.success("Sales Entry Saved ✅ (Autosaved with IP)")
