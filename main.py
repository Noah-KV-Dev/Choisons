...
for f,p in fuel_defaults.items():
...
if page=="Sales Entry":
    ...
    if not st.session_state.checklist_done:
        st.error("❌ Sales Entry Blocked — Complete Staff Daily Checklist First")
        st.stop()

    staff_list=pd.read_sql("SELECT name FROM staff",conn)["name"].tolist()

    if len(staff_list)==0:
        st.warning("No staff added")
        st.stop()
    ...
    with col1:
        time_in=st.time_input("Duty Time IN")

    with col2:
        time_out=st.time_input("Duty Time OUT")
    ...
    st.info(f"Working Hours: {hours} hrs")
    ...
    if len(today)>0:
        ...
        st.subheader("Staff Wise Litre Graph (Today)")
...
# ---------------- REPORTS ----------------
    ...
    if report_type=="Daily Report":
        ...
        st.write("Total Sales ₹",r["total"].sum())
        ...
        m=st.selectbox("Select Month",sorted(df["month"].unique()))
        ...
        p=df[["paytm","sbi","hppay","advance","creditor","balance"]].sum()
